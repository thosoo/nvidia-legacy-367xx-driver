...
`completed_sequence >= barrier`. Completion is published only after the worker has
dequeued the item, invoked the callback, observed callback return, performed
queue-owned cleanup, and advanced `completed_sequence`. The first barrier drains work accepted before
...
| `DRAINING` | rejected | accepted for callback-generated/static requeue work after producer quiescence | two barriers | additional callers wait on generation | returns `-EDEADLK` |
| `STOPPING` | rejected | rejected | should be empty after owner drain | additional callers wait on generation | returns `-EDEADLK` |
